const navLinks = [
  {
    name: "Work",
    link: "#work",
  },
  {
    name: "Experience",
    link: "#experience",
  },
  {
    name: "Contact",
    link: "#contact",
  },
];

const words = [
  { text: "Ideas", imgPath: "/images/ideas.svg" },
  { text: "Concepts", imgPath: "/images/concepts.svg" },
  { text: "Designs", imgPath: "/images/designs.svg" },
  { text: "Code", imgPath: "/images/code.svg" },
  { text: "Ideas", imgPath: "/images/ideas.svg" },
  { text: "Concepts", imgPath: "/images/concepts.svg" },
  { text: "Designs", imgPath: "/images/designs.svg" },
  { text: "Code", imgPath: "/images/code.svg" },
];


const counterItems = [
  { value: 13, suffix: "+", label: "Months of Experience" },
  { value: 35, suffix: "+", label: "Completed Projects" },
  { value: 200000, suffix: "+", label: "Lines of Code" },
];

const logoIconsList = [
  {
    imgPath: "/images/logos/tsz.png",
  },
  {
    imgPath: "/images/logos/mathnasium.png",
  },
  {
    imgPath: "/images/logos/company-logo-4.png",
  },
  {
    imgPath: "/images/logos/company-logo-9.png",
  },
  {
    imgPath: "/images/logos/tsz.png",
  },
  {
    imgPath: "/images/logos/mathnasium.png",
  },
  {
    imgPath: "/images/logos/company-logo-4.png",
  },
  {
    imgPath: "/images/logos/company-logo-9.png",
  },
];

const abilities = [
  {
    imgPath: "/images/seo.png",
    title: "Quality Focus",
    desc: "Delivering high-quality results while maintaining attention to every detail.",
  },
  {
    imgPath: "/images/chat.png",
    title: "Reliable Communication",
    desc: "Keeping the team updated at every step to ensure transparency and clarity.",
  },
  {
    imgPath: "/images/time.png",
    title: "On-Time Delivery",
    desc: "Making sure projects are completed on schedule, with quality & attention to detail.",
  },
];

const techStackImgs = [
  {
    name: "React Developer",
    imgPath: "/images/logos/react.png",
  },
  {
    name: "Python Developer",
    imgPath: "/images/logos/python.svg",
  },
  {
    name: "Backend Developer",
    imgPath: "/images/logos/node.png",
  },
  {
    name: "Interactive Developer",
    imgPath: "/images/logos/three.png",
  },
  {
    name: "Java Developer", // For Spring Boot, Java
    imgPath: "/images/logos/java.png",
  },
  {
    name: "Database Developer", // For PostgreSQL, MariaDB, SQL
    imgPath: "/images/logos/database.png",
  },
  {
    name: "DevOps Engineer", // For Docker, CI/CD
    imgPath: "/images/logos/devops.png",
  },
  {
    name: "Next.js Developer",
    imgPath: "/images/logos/nextjs.png",
  },
];


const expCards = [
  {
    review: "",
    imgPath: "/images/logos/mathnasium.png",
    logoPath: "/images/math.png",
    title: "Instructor",
    date: "Aug 2023 - present",
    responsibilities: [
      "Instructed students in high school mathematics, including algebra, geometry, trigonometry, and calculus, with a focus on conceptual clarity and problem-solving.",

"Taught web development fundamentals using HTML, CSS, JavaScript, Python, and SQL through interactive projects and real-world examples.",

"Developed customized lesson plans and provided individual support to help students build confidence and achieve academic goals.",
    ],
  },
  {
    review: "",
    imgPath: "/images/logos/tsz.png",
    logoPath: "/images/TSZL.png",
    title: "IT Support Analyst",
    date: "Sep 2024 - Apr 2025",
    responsibilities: [
      "Resolved escalated Level 1 tickets involving OS failures, application crashes, hardware malfunctions, and connectivity issues.",
      "Provided on-site and remote deskside support for desktops, laptops, printers, and peripherals, ensuring minimal user downtime.",
      "Managed user accounts, permissions, and access using Active Directory, following least-privilege principles.",
      "Performed system imaging, upgrades, and software deployments on Windows systems.",
      "Maintained ticket documentation and asset updates in Jira, aligned with ITIL incident and change workflows.",
    ],
  },

  {
    review: "",
    imgPath: "/images/logos/tsz.png",
    logoPath: "/images/TSZL.png",
    title: "IT Support & Systems Analyst Intern",
    date: "May 2024 - Aug 2024 · Internship",
    responsibilities: [
      "Provided Level 1 technical support for Windows 11 upgrade tickets, troubleshooting user issues and escalating when necessary.",
      "Monitored and supported business applications by diagnosing common issues and resolving or escalating incidents appropriately.",
      "Assisted with system installation, configuration, and testing to ensure proper functionality and identify malfunctions.",
      "Troubleshot hardware and software issues, including system errors and equipment failures, using standard diagnostic procedures.",
      "Supported project coordination by tracking progress, assisting with technical tasks, and communicating updates to stakeholders.",
      "Maintained system performance by identifying recurring application issues and recommending improvements.",
      "Documented incidents, resolutions, and processes to improve knowledge sharing and service efficiency.",
    ],
  },
  
  {
    review: "",
    imgPath: "/images/logos/out_two.jpeg",
    logoPath: "/images/logos/out_two.jpeg",
    title: "Quality Assurance Contractor",
    date: "May 2025 - present",
    responsibilities: [
      "Reviewed 50, 000+ LLM Python/Java code, resolving architectural flaws and vulnerabilities, boosting quality",
 "Developed 500+ Pytest/JUnit test cases, achieving 90-100% coverage. Ensured correct data sets for AI models.",
 "Implemented Python/Java code fixes/optimizations from reviews and testing. Reducing critical defects upto 39%",
    ],
  },
];

const expLogos = [
  {
    name: "logo1",
    imgPath: "/images/logo1.png",
  },
  {
    name: "logo2",
    imgPath: "/images/logo2.png",
  },
  {
    name: "logo3",
    imgPath: "/images/logo3.png",
  },
];


const socialImgs = [
  {
    name: "Github",
    imgPath: "/images/gHub.png",
    url: 'https://github.com/svingh',
  },
  {
    name: "linkedin",
    imgPath: "/images/linkedin.png",
    url: 'https://www.linkedin.com/in/vikramjeet-singh-61b562221/',
  },
];

export {
  words,
  abilities,
  logoIconsList,
  counterItems,
  expCards,
  expLogos,
  socialImgs,
  techStackImgs,
  navLinks,
};